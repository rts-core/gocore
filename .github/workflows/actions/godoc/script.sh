...
if [ $ignore_src = 'true' ] ; then
  wget -m -k --reject go --show-progress --progress=dot -erobots=off --no-host-directories --no-use-server-timestamps -X /debug http://localhost:8080
else
  wget -m -k --show-progress --progress=dot -erobots=off --no-host-directories --no-use-server-timestamps -X /debug http://localhost:8080
fi
